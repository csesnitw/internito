...
const AddExperience = ({ initialExperience, editMode, experienceId }) => {
  ...
  const areRoundsValid = experience.interviewRounds.every((round) => 
    round.title.trim() !== "" && 
    round.type.trim() !== "" && 
  ...
  const handleSubmit = async (e) => {
    ...
    const validationChecks = [
      ...
      { condition: !experience.cgpaCutoff || experience.cgpaCutoff < 0 || experience.cgpaCutoff > 10, 
      ...
      { condition: !dontRememberSelections && (!experience.numberOfSelections || experience.numberOfSelections < 0), 
        message: "Please enter a valid number of selections." },
    ];
    ...
    const payload = {
      ...
      rounds: experience.rounds.length ? experience.rounds : [],
      };
      ...
      <form className="add-exp-form" onSubmit={handleSubmit}>
        ...
        <div className="add-exp-section">
          ...
          <div className="add-exp-ot-questions-list">
            {experience.OT_questions.map((q, idx) => (
              <div
                key={idx}
                className={`add-exp-ot-question-card ${
                  submitAttempted ? (q.trim() ? "valid" : "invalid") : ""
                }`}
              >
                <input
                  type="text"
        ...
        <div className="add-exp-section">
          ...
          {experience.interviewRounds.map((round, idx) => {
            ...
            const roundClass = `add-exp-round${
              submitAttempted ? (isRoundValid ? " valid" : " invalid") : ""
            }`;
